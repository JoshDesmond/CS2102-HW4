...
class EarthquakeExamples {
    ...
    @TestMethod
    public boolean testEarthquake2(final Tester t) {
    ...
    @TestMethod
    public boolean testEarthQuake(final Tester t) {
        LinkedList<Double> sensorData = getSensorData();

        if (!t.checkExpect(E.dailyMaxForMonth(sensorData, 5),
                getOutputData5())) {
            return t.fail();
        }

        // now do another test with same data, ensuring it isn't destroyed:
        return t.checkExpect(E.dailyMaxForMonth(sensorData, 10),
                getOutputData10());
    }
    ...
    private static LinkedList<Double> getSensorData() {
        ...
        final LinkedList<Double> data = new LinkedList<Double>(
                Arrays.asList(20150508., 100., // 100
                        20150509., 100., // 100
